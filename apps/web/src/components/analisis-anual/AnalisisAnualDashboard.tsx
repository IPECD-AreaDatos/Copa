...
export default function AnalisisAnualDashboard() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchWithAuth(`/copa/copa-api/api/ron/annual-monitor?ts=${Date.now()}`, {
      cache: "no-store",
      headers: {
      ...
      .then((response) => {
        ...
        return response.json() as Promise<AnnualMeta>;
      })
      .then((next) => {
        if (cancelled) return;
        setPayload(next);
        setYearId(next.annual_monitor.meta.default_period_id || "");
      })
      .catch((error: unknown) => {
        if (!cancelled) setErr(error instanceof Error ? error.message : "Error al cargar datos.");
      });
    return () => {
      cancelled = true;
    };
    ...
    <div className="annual-dashboard">
      ...
      <div style={{ color: "var(--text-secondary)", marginBottom: "1.5rem", fontSize: "0.875rem" }}>
        ...
        <p>{periodRow.kpi.meta?.source === "monthly"
          ? "Fuente: misma base de datos del análisis mensual."
      ...
      <section className="section-group" style={{ marginTop: "2rem" }}>
        ...
        <div className="hero-grid-flex">
          <article className="kpi-card" style={{ borderTop: `4px solid ${getBorderColorByValue(vm.masa.current)}` }}>
            <div
              className="info-tooltip"
              data-tooltip={`Muestra la masa salarial total liquidada acumulada en el año, en millones de pesos corrientes, correspondiente al período seleccionado.
...
La masa salarial total incluye los conceptos de salarios,  plus y bonos para los 3 poderes del estado.`}
          ...
          <article className="kpi-card" style={{ borderTop: `4px solid ${getBorderColorByValue(vm.masa.prev)}` }}>
            <div
              className="info-tooltip"
              data-tooltip={`Muestra la masa salarial total liquidada acumulada en el año, en millones de pesos corrientes, correspondiente al año anterior al seleccionado.
...
El valor de los Recursos de Origen Nacional disponibles, surge del RON total descontado los recursos con afectación específica y el porcentaje coparticipable con los municipios. Es decir  incluye la suma de los conceptos de: C.F.I. Neta de Ley N° 26.075, Financiamiento Educativo Ley N° 26.075, Régimen Simplificado para Pequeños Contribuyentes Ley N° 24.977 y Compensación Consenso Fiscal menos el 19% que se redistribuye a municipios.`}
      ...
      <section className="section-group" style={{ marginTop: "2rem" }}>
        <div className="chart-container" style={{ padding: "1rem 3%" }}>
          ...
          <div
            className="info-tooltip"
            data-tooltip={`Comparación de la evolución mensual de los ingresos provinciales provenientes de los Recursos de Origen Nacional (RON) en millones de pesos en términos nominales para el año actual seleccionado y el año anterior.
